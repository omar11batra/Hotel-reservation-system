...
$(function() {
	...
	var dt_codes_table = $('.datatables-codes'), //name of the table
		select2 = $('.select2'), //??
		userView = 'app-user-view-account.html'; //for the button view details

	// Users datatable
	if (dt_codes_table.length) {
		var dt_codes = dt_codes_table.DataTable({
			ajax: assetsPath + 'json/codes.json', // JSON file to add data
			columns: [
			...
			columnDefs: [
...

				{
					...
					render: function(data, type, full, meta) {
						...
						return '<span class="fw-light">' + $type + '</span>';
					}
				},

				{
					...
					render: function(data, type, full, meta) {
						return (
							...
							'<a href="javascript:;" data-bs-toggle="offcanvas" data-bs-target="#offcanvaEditCodes"' +
			...
			buttons: [
...

				{
					...
					attr: {
						...
						'data-bs-target': '#offcanvasAddExternalCodes'
					}
				}
			],
	...
	// Delete Record
	$('.datatables-codes tbody').on('click', '.delete-record', function() {
...
// Validation 
(function () {
    var AddcodeForm = document.getElementById('addExternalCodeForm');


    // Add New User Form Validation
    const fv = FormValidation.formValidation(AddcodeForm, {
        fields: {
            Code: {
             validators: {
              notEmpty: {
